...
**Spanish articles, contractions and prepositions are NOT part of the term.**
Spanish text almost always presents a noun phrase with a determiner or a
contraction glued to its front. Start the span at the noun:

- text `del genocidio` -> term `genocidio`
- text `la inteligencia artificial` -> term `inteligencia artificial`
- text `de la cadena de suministro` -> term `cadena de suministro`
- text `a las resoluciones pertinentes` -> term `resoluciones pertinentes`

Keep an internal preposition that belongs to the term itself (`cadena de
suministro`, `derechos humanos`) -- cutting there would break the term. Keep the
printed inflection exactly as it appears, and never strip an accent or reattach an
article that is not there. The span must still be copied character for character
from the sentence.

**Coordinated nouns: emit one term per conjunct.** When the sentence coordinates
two nouns and each carries its own article (`la paz y el desarrollo`, `la equidad y la justicia`), the whole string cannot
be a clean term -- the interior article would sit inside it. Emit each conjunct as
its own pair instead (`和平`/`paz` and `发展`/`desarrollo`). Emit the coordinated whole only when
the text prints it without an interior article (`paz y desarrollo`).

This does not apply when the Chinese side is an abbreviation that cannot be split:
`教体文卫` has no shorter Chinese substring that is a term, so pairing it with the
full enumeration (`la educación, el deporte, la cultura y la salud`) is correct.

**Chinese spans must not swallow a 的-modifier.** Chinese marks modification with
an infix 的, so unlike a Spanish article you cannot simply drop it: the
...
2. If no shorter substring works, keep the whole span ONLY when it is a fixed
   ...
   (`parte integrante`), `生活的质量` (`calidad de vida`), `真正的多边主义`.
