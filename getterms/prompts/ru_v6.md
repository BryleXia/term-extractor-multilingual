...
- Russian terms almost always appear in an oblique case. **Copy the inflected
  ...
  `dict_form`, described in the next section — that is where the dictionary form
  goes.
...
happened to use. This follows ISO 10241-1 (a term is given in its basic
grammatical form: a noun in the singular) and the IATE handbook (nouns and
adjectives in the singular, *except where the term is habitually used in the
plural*). So besides the verbatim spans you must return `dict_form`: the
...
stay verbatim, exactly as the section above demands. `dict_form` is the restored
...
- **Also put the head in the SINGULAR.** `ядерных ударов` -> `ядерный удар`,
  `диких кабанов` -> `дикий кабан`, `коленчатых валов` -> `коленчатый вал`.
  The modifiers agree with the singular head.
- **But keep the plural when the term is HABITUALLY USED in the plural.** The
  test is NOT "does a singular exist" — for most of the terms below a singular
  exists and is perfectly good Russian. The test is **"how is this term normally
  written in a dictionary or a terminology database"**. Do not singularize:
  * pluralia tantum: `переговоры`, `сутки`
  * fixed terms: `совместные учения` (military exercises),
    `права человека` (human rights)
  * terms that their own field writes in the plural: `атмосферные осадки`
    (meteorology), `рынки капитала`, `национальные интересы`, `рабочие места`,
    `запасы`, `доходы бюджета`, `расходы бюджета`, `народы всего мира`
  Proper names and abbreviations never change either.
- **When you cannot tell whether a term is normally singular or plural, KEEP THE
  NUMBER THE SENTENCE USED** (still put it in the nominative). Do not guess.
  Leaving a term in the sentence's own number is a small loss; moving it to the
  wrong number puts a wrong entry in the term bank. This applies to the HEAD
  word, not only to subordinate nouns.
...
- **If the span is already the dictionary form (nominative AND the right
  number), copy it unchanged.** That is the expected answer for `Запад` and
  `большая панда`. A nominative plural is NOT automatically the dictionary form:
  `политические заверения` still becomes `политическое заверение`, and
  `цифровые инструменты` becomes `цифровой инструмент` — **unless the term is one
  of the habitually-plural ones listed above**, in which case the plural stays.
  Never leave `dict_form` empty.
- An upstream misspelling stays misspelled here too. Restore the case, nothing
  else.

`dict_form` always describes the RUSSIAN side, whichever slot it happens to sit in:
...
changes and it is never what `dict_form` reports.
...
| `ядерных ударов` | `ядерный удар` | genitive plural -> nominative **singular** |
| `совместных учений` | `совместные учения` | nominative; **plural kept — this term is habitually plural** |
| `диких кабанов` | `дикий кабан` | plural -> singular, adjective agrees |
| `права человека` | `права человека` | habitually plural, unchanged |
| `атмосферных осадков` | `атмосферные осадки` | genitive -> nominative; **plural kept — meteorology writes it plural** |
| `рынках капитала` | `рынки капитала` | prepositional -> nominative; plural kept (es/fr write this term plural too) |
| `рабочих мест` | `рабочие места` | genitive -> nominative; plural kept — the singular means "one workstation" |
...
| `политические заверения` | `политическое заверение` | already nominative, but plural -> singular |
...
[
  {"sent_id": 1, "terms": [
    {"term_src": "世界防法西斯战争", "term_tgt": "войне с нацизмом", "dict_form": "война с нацизмом", "types": ["poli", "cul"], "note": ""}
  ]},
  {"sent_id": 2, "terms": [
    {"term_src": "президент Путин", "term_tgt": "普京总统", "dict_form": "президент Путин", "types": ["per"], "note": ""},
    {"term_src": "империю лжи", "term_tgt": "谎言帝国", "dict_form": "империя лжи", "types": ["tab", "poli"], "note": ""},
    {"term_src": "Запад", "term_tgt": "西方", "dict_form": "Запад", "types": ["poli"], "note": ""}
  ]},
  {"sent_id": 3, "terms": [
    {"term_src": "военного блока НАТО", "term_tgt": "北约", "dict_form": "военный блок НАТО", "types": ["org"], "note": ""},
    {"term_src": "политические заверения", "term_tgt": "政治保证", "dict_form": "политическое заверение", "types": ["poli"], "note": ""}
  ]},
  {"sent_id": 4, "terms": [
    {"term_src": "внеблокового статуса", "term_tgt": "不结盟地位", "dict_form": "внеблоковый статус", "types": ["poli"], "note": ""},
    {"term_src": "Украины", "term_tgt": "乌克兰", "dict_form": "Украина", "types": ["loc"], "note": ""}
  ]},
  {"sent_id": 5, "terms": [
    {"term_src": "русофобского киевского режима", "term_tgt": "恐俄基辅政权", "dict_form": "русофобский киевский режим", "types": ["tab", "poli"], "note": ""},
    {"term_src": "кровавого госпереворота", "term_tgt": "血腥政变", "dict_form": "кровавый госпереворот", "types": ["tab", "poli"], "note": ""}
  ]},
  {"sent_id": 6, "terms": [
    {"term_src": "加关税", "term_tgt": "повышение таможенных пошлин", "dict_form": "повышение таможенных пошлин", "types": ["econ"], "note": ""}
  ]},
  {"sent_id": 7, "terms": [
    {"term_src": "人工智能", "term_tgt": "искусственного интеллекта", "dict_form": "искусственный интеллект", "types": ["tech", "hot"], "note": ""},
    {"term_src": "数字工具", "term_tgt": "цифровые инструменты", "dict_form": "цифровой инструмент", "types": ["tech"], "note": ""}
  ]},
  {"sent_id": 8, "terms": [
    {"term_src": "生船机", "term_tgt": "судоподъёмник", "dict_form": "судоподъёмник", "types": ["tech"], "note": ""}
  ]},
  {"sent_id": 9, "terms": [
    {"term_src": "большая панда", "term_tgt": "大熊猫", "dict_form": "большая панда", "types": ["other"], "note": ""}
  ]},
  {"sent_id": 10, "terms": []}
]
```
...
- Examples 2, 4 and 5 show the inflection rule. `империю лжи` is accusative,
  ...
  `русофобский киевский режим` appear **in `dict_form`** on the very same objects.
  Compare example 3: `политические заверения` is already nominative, so its
  `dict_form` is identical to the span only when the span is already nominative
  **and** already the right number — otherwise the number is fixed too.
...
- In example 7, `人工智能` is `tech` **and** `hot`, because AI vocabulary rose
  ...
  `dict_form` carries `искусственный интеллект`. This example also shows that
  `dict_form` follows the RUSSIAN side even when Russian sits in the `tgt` slot.
...
- terms: array of objects:
  - term_src
  - term_tgt
  - dict_form  (the Russian side restored to the nominative, as described above;
