...
Keep the inflected form exactly as printed: do not change `résolutions
pertinentes` to `résolution pertinente`. The span must still be copied character for character from the
...
- **Keep the plural when the term is HABITUALLY USED in the plural.** The test is
  NOT "does a singular exist" — for most of the terms below a singular exists and
  is perfectly good French. The test is **"how is this term normally written in a
  dictionary or a terminology database"**. Do not singularize:
  * fixed multiword terms: `droits de l'homme`, `arts et métiers traditionnels`
  * official names and set formulations, **including ones made of ordinary
    nouns**: `Nations Unies`, `objectifs de développement durable` (the UN
    framework: 17 goals, the name itself is plural)
  * terms that their own field writes in the plural: `précipitations`
    (meteorology), `dépenses publiques`, `recettes fiscales`, `infrastructures`,
    `nouilles instantanées`, `vermicelles`, `temps anciens`
  ⚠ For some of these the singular **is a different word**: `précipitation`
  mainly means "haste", `recette` commonly means "a recipe", and `une nouille` in
  the singular colloquially means "a dimwit" (Académie française, 9th ed., sense
  2) -- the food itself is written in the plural. If the singular would change the
  meaning, the plural IS the dictionary form.
- **When you cannot tell whether a term is normally singular or plural, KEEP THE
  FORM THE SENTENCE USED.** Do not guess. Leaving a term in the sentence's own
  number is a small loss; moving it to the wrong number puts a wrong entry in the
  term bank. **This applies to the HEAD noun, not only to complements.**
- **Proper names, institution names and abbreviations never change.**
  `Nations Unies`, `Conseil de sécurité`, `Organisation des Nations Unies`,
  `COVID-19`, `ONU` stay exactly as printed, plural included.
- **A complement noun takes the number the ESTABLISHED TERM has — not the number
  it happens to carry in the sentence.** In French a complement noun does not
  agree with its head (`un chef-d'œuvre` -> `des chefs-d'œuvre`,
  `un timbre-poste` -> `des timbres-poste`), so its number is a lexical property
  of the term and it carries meaning. Decide which of these two it is:
  **First check whether the complement carries an article.** If it does --
  `du`, `de la`, `de l'`, `des` -- it points at a determinate set or entity and
  its number is fixed by what it refers to: **copy that number unchanged and do
  not ask whether it is a category**. `emploi des femmes`, `mobilité des
  travailleurs`, `monopole des armes`, `contrôle des crues`, `allocation des
  ressources`, `marché des actions`, `revenu des ménages`, `marché des capitaux`,
  `eau de fonte des glaciers` all keep exactly the number they are printed with.
  Only the head noun moves. The two cases below apply **only** to a bare
  complement with no article (`de` / `d'` / `à base de` + noun):
  * the complement names the **constituent parts** of a collective, a container
    or a set -> **keep the plural**: `immeuble de bureaux` (a building holds many
    offices), `chaîne de montagnes`, `groupement d'entreprises`,
    `banc de poissons`. Making these singular is a real error — do not do it.
  * the complement names a **category, a material or a purpose** -> **singular**:
    `nouvelle forme d'entreprise` (the category "entreprise", not several named
    companies), `racine de lotus`, `aliment à base de farine`,
    `champignon de pin` (the kind of tree, not several pines),
    `bateau de croisière` (what the boat is for).
  So `nouvelles formes d'entreprises` -> `nouvelle forme d'entreprise`: the head
  goes singular AND the complement is a category. But
  `immeubles de bureaux` -> `immeuble de bureaux`: only the head moves.
  When in doubt, prefer the form a French dictionary or a terminology database
  would print for that exact term, and if you cannot tell, leave the complement
  as it stands rather than guess. (This doubt only ever arises for a bare
  complement; an article-carrying complement is always copied unchanged.)
...
| `nouvelles formes d'entreprises` | `nouvelle forme d'entreprise` | head singular, **feminine kept**; the complement is a CATEGORY, so it goes singular too |
| `immeubles de bureaux` | `immeuble de bureaux` | head singular; the complement names the constituents, so the plural STAYS |
| `chaînes de montagnes` | `chaîne de montagnes` | same — a range is made of several mountains |
| `revenus des ménages` | `revenu des ménages` | same |
| `groupements d'entreprises` | `groupement d'entreprises` | same — a grouping is made of several companies |
...
| `précipitations atmosphériques` | `précipitations atmosphériques` | habitually plural; the singular `précipitation` means "haste" |
| `objectifs de développement durable` | `objectifs de développement durable` | official UN framework name — plural is part of the name |
| `dépenses publiques` | `dépenses publiques` | aggregate written in the plural in its field |
| `nouilles instantanées` | `nouilles instantanées` | a food written in the plural; the singular `une nouille` means "a dimwit" |
...
[
  ...
  {"sent_id": 4, "terms": [
    ...
    {"term_src": "新业态", "term_tgt": "nouvelles formes d'entreprises", "dict_form": "nouvelle forme d'entreprise", "types": ["hot", "econ"], "note": ""}
  ]},
  {"sent_id": 5, "terms": [
...
Points to notice:
- The `dict_form` values show the cases side by side: `forces terroristes`
  -> `force terroriste`, `dettes en souffrance` -> `dette en souffrance` and
  `nouvelles formes d'entreprises` -> `nouvelle forme d'entreprise` are restored
  — note that in the last one BOTH the head and the complement go singular,
  because `entreprise` names a category; `Nations Unies` and
  `arts et métiers traditionnels` keep their plural because the term is a proper
  name or a set expression; the rest are already dictionary forms and are
  repeated unchanged.
