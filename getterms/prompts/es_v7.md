...
So besides the verbatim spans you must return `dict_form`: the SPANISH side of
the pair in its dictionary form. Chinese is not inflected, so `dict_form` never
...
- Put the HEAD noun in the SINGULAR and make its adjectives agree with it:
  `empresas privadas` -> `empresa privada`, `luces de neón` -> `luz de neón`.
- **NEVER change the gender of a noun.** Gender is an inherent property of a
  Spanish noun, not something to normalize: `la política` (policy) and `el
  político` (politician) are different words. Write `empresa privada`, never
  `empresa privado`. An adjective agrees with its head noun; it does not go to
  the masculine just because a dictionary lists it that way.
- **Keep the plural when the term is HABITUALLY USED in the plural.** The test is
  ...
  is perfectly good Spanish. The test is **"how is this term normally written in a
  dictionary or a terminology database"**. Do not singularize:
  * fixed multiword terms: `derechos humanos`, `fuerzas armadas`,
    `elecciones libres e imparciales`
  * official names and set formulations, including ones made of ordinary nouns:
    `Naciones Unidas`, `Estados Unidos`, `Objetivos de Desarrollo Sostenible`,
    `nuevas fuerzas productivas de calidad`
  * terms that their own field writes in the plural: `precipitaciones`
    (meteorology), `elecciones`, `recursos hídricos`, `ingresos fiscales`,
    `gastos públicos`, `infraestructuras`, `masas populares`,
    `pueblos originarios`, `tiempos remotos`, `palillos`, `flores y pájaros`
  ⚠ For some of these the singular **is a different word**: `precipitación`
  mainly means "haste", `elección` means "a choice". If the singular would change
  the meaning, the plural IS the dictionary form.
...
- **Proper names and abbreviations never change.** `Estados Unidos`,
  `Buenos Aires`, `Naciones Unidas`, `ONU` stay exactly as printed.
- **A subordinate noun keeps its own form.** In `injerencia en los asuntos
  internos` only the head could change; `los asuntos internos` stays as it is.
  Same for `salas de exposición` -> `sala de exposición`, where `de exposición`
  is untouched.
- **Articles and prepositions inside the span are copied as they are.** They do
  not participate in the restoration.
- **Never change the number of words.** Do not translate, add or drop a word, or
  expand an abbreviation.
- A verb stays in the infinitive (`profundizar la reforma` is already the
  dictionary form).
...
|---|---|---|
| `sanciones unilaterales` | `sanción unilateral` | plural -> singular, adjective agrees |
| `salas de exposición` | `sala de exposición` | head singular; `de exposición` untouched |
| `empresas privadas` | `empresa privada` | adjective agrees, **feminine kept** |
| `luces de neón` | `luz de neón` | irregular plural `luces` -> `luz` |
| `instituciones sanitarias` | `institución sanitaria` | accent restored with the singular |
| `grandes modelos` | `gran modelo` | `grande` apocopates before a singular noun |
| `derechos humanos` | `derechos humanos` | habitually plural, unchanged |
| `precipitaciones atmosféricas` | `precipitaciones atmosféricas` | habitually plural; the singular `precipitación` means "haste" |
| `elecciones libres e imparciales` | `elecciones libres e imparciales` | habitually plural; the singular `elección` means "a choice" |
| `recursos hídricos` | `recursos hídricos` | aggregate written in the plural in its field |
| `Estados Unidos` | `Estados Unidos` | proper name, unchanged |
| `nuevas fuerzas productivas de calidad` | `nuevas fuerzas productivas de calidad` | set formulation, unchanged |
| `economía mundial` | `economía mundial` | already the dictionary form |
...
[
  {"sent_id": 1, "terms": [
    {"term_src": "改革开放", "term_tgt": "reforma y apertura", "dict_form": "reforma y apertura", "types": ["poli", "cul"], "note": ""},
    {"term_src": "制度型开放", "term_tgt": "apertura institucional", "dict_form": "apertura institucional", "types": ["poli", "econ"], "note": ""}
  ]},
  {"sent_id": 2, "terms": [
    {"term_src": "Secretario General de las Naciones Unidas", "term_tgt": "联合国秘书长", "dict_form": "Secretario General de las Naciones Unidas", "types": ["poli"], "note": ""},
    {"term_src": "Naciones Unidas", "term_tgt": "联合国", "dict_form": "Naciones Unidas", "types": ["org"], "note": ""},
    {"term_src": "Papa León XIV", "term_tgt": "教皇十四世里奥", "dict_form": "Papa León XIV", "types": ["per"], "note": ""},
    {"term_src": "Nueva York", "term_tgt": "纽约", "dict_form": "Nueva York", "types": ["loc"], "note": ""}
  ]},
  {"sent_id": 3, "terms": [
    {"term_src": "单边制裁", "term_tgt": "sanciones unilaterales", "dict_form": "sanción unilateral", "types": ["tab", "poli"], "note": ""},
    {"term_src": "干涉内政", "term_tgt": "injerencia en los asuntos internos", "dict_form": "injerencia en los asuntos internos", "types": ["tab", "poli"], "note": ""}
  ]},
  {"sent_id": 4, "terms": [
    {"term_src": "新质生产力", "term_tgt": "nuevas fuerzas productivas de calidad", "dict_form": "nuevas fuerzas productivas de calidad", "types": ["hot", "econ"], "note": ""},
    {"term_src": "大模型", "term_tgt": "grandes modelos", "dict_form": "gran modelo", "types": ["tech", "hot"], "note": ""},
    {"term_src": "世界经济", "term_tgt": "economía mundial", "dict_form": "economía mundial", "types": ["econ"], "note": ""}
  ]},
  {"sent_id": 5, "terms": [
    {"term_src": "导览路线", "term_tgt": "ruta de visita", "dict_form": "ruta de visita", "types": ["other"], "note": ""},
    {"term_src": "展厅", "term_tgt": "salas de exposición", "dict_form": "sala de exposición", "types": ["other"], "note": ""}
  ]},
  {"sent_id": 6, "terms": []}
]
```
...
Points to notice:
- The `dict_form` values show the three cases side by side: `sanciones
  unilaterales` -> `sanción unilateral` and `salas de exposición` -> `sala de
  exposición` are restored; `nuevas fuerzas productivas de calidad` and
  `Naciones Unidas` keep their plural because the term is habitually plural or a
  proper name; the rest are already dictionary forms and are repeated unchanged.
