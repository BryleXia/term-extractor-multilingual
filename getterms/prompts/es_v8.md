...
- **Keep the plural when the term is HABITUALLY USED in the plural.** The test is
  ...
  * terms that their own field writes in the plural: `elecciones`,
    `recursos hídricos`, `ingresos fiscales`, `masas populares`,
    `pueblos originarios`, `tiempos remotos`, `palillos`, `flores y pájaros`
  ⚠ For some of these the singular **is a different word**: `elección` means
  "a choice", `palillo` means "a toothpick", `masa` means "mass" in physics. If
  the singular would change the meaning, the plural IS the dictionary form.
...
| `palillos` | `palillos` | habitually plural; the singular `palillo` means "a toothpick" |
