...
- **Keep the plural when the term is HABITUALLY USED in the plural.** The test is
  ...
  * terms that their own field writes in the plural: `précipitations`
  ...
  mainly means "haste", `recette` commonly means "a recipe", `une nouille` means
  "a fool" (familiar) -- the pasta is `des nouilles`. If the singular would change
...
- **A complement noun takes the number the ESTABLISHED TERM has — not the number
  ...
  * the complement names the **constituent parts** of a collective, a container
    ...
    `banc de poissons`, `marché des capitaux`, `revenu des ménages`,
    `cérémonie en l'honneur du lac et des poissons`. Making these singular is a
    real error — do not do it.
  * the complement names a **category, a material or a purpose** -> **singular**:
  ...
  as it stands rather than guess.
...
| `nouilles instantanées` | `nouilles instantanées` | a food written in the plural; `une nouille` is one noodle |
