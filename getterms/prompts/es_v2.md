...
[
  ...
  {"sent_id": 2, "src": "El Papa León XIV visitó la sede de las Naciones Unidas en Nueva York.", "tgt": "教皇十四世里奥访问了位于纽约的联合国总部。"},
  ...
  {"sent_id": 5, "src": "谢谢大家，我们现在开始。", "tgt": "Gracias a todos, comencemos."}
]
```
...
[
  ...
  {"sent_id": 4, "terms": [
    ...
    {"term_src": "世界经济", "term_tgt": "economía mundial", "types": ["other", "econ"], "note": ""}
  ]},
  {"sent_id": 5, "terms": []}
]
```

Note in example 2 that `src` is Spanish and `tgt` is Chinese, and the term sides
follow the slots, not the languages. Note in example 4 that `新质生产力` is `hot`
because it is a recent formulation, while `世界经济` is `other` because it is a
long-settled concept that fits no single domain. Note in example 5 that a
sentence with no candidates gets an empty array, not a fabricated term.
