...
two nouns and each carries its own article (`l'équité, la justice`, `la paix et le développement`), the whole string cannot
be a clean term -- the interior article would sit inside it. Emit each conjunct as
its own pair instead (`公平`/`équité` and `正义`/`justice`). Emit the coordinated whole only when
the text prints it without an interior article (`équité et justice`).
...
full enumeration (`l'éducation, du sport, de la culture et de la santé`) is correct.
