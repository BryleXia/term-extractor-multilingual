...
- **But keep the plural when the term is habitually plural.** Some terms only
  exist in the plural, and there the plural IS the dictionary form:
  `совместные учения` (military exercises), `права человека` (human rights),
  `переговоры`, `сутки`. Proper names and abbreviations never change either.
...
- **If the span is already the dictionary form (nominative AND the right
  ...
  `цифровые инструменты` becomes `цифровой инструмент`. Never leave
  `dict_form` empty.
